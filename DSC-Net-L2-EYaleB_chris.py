...
class ConvAE(object):
    ...
    def _initialize_weights(self):
        all_weights = dict()
        all_weights['enc_w0'] = tf.get_variable("enc_w0", shape=[self.kernel_size[0], self.kernel_size[0], 1, self.n_hidden[0]], #[5,5,1,10]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['enc_b0'] = tf.Variable(tf.zeros([self.n_hidden[0]], dtype = tf.float32))

        all_weights['enc_w1'] = tf.get_variable("enc_w1", shape=[self.kernel_size[1], self.kernel_size[1], self.n_hidden[0],self.n_hidden[1]], #[3,3,10,20]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['enc_b1'] = tf.Variable(tf.zeros([self.n_hidden[1]], dtype = tf.float32))

        all_weights['enc_w2'] = tf.get_variable("enc_w2", shape=[self.kernel_size[2], self.kernel_size[2], self.n_hidden[1],self.n_hidden[2]], #[3,3,20,30]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['enc_b2'] = tf.Variable(tf.zeros([self.n_hidden[2]], dtype = tf.float32))

        #adding a third encoder layer weights here
        all_weights['enc_w3'] = tf.get_variable("enc_w3", shape=[self.kernel_size[3], self.kernel_size[3], self.n_hidden[2],self.n_hidden[3]], #[2,2,30,30]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        ...
        #-------------------------- START OF DECODER -------------------------
        #adding a third decoder layer weights here (at the start of decoding)
        all_weights['dec_w0'] = tf.get_variable("dec_w0", shape=[self.kernel_size[3], self.kernel_size[3], self.n_hidden[2],self.n_hidden[3]], #[2,2,30,30]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['dec_b0'] = tf.Variable(tf.zeros([self.n_hidden[2]], dtype = tf.float32))
        #---------------------------------------------------------------------------
        all_weights['dec_w1'] = tf.get_variable("dec_w1", shape=[self.kernel_size[2], self.kernel_size[2], self.n_hidden[1],self.n_hidden[2]], #[3,3,20,30]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['dec_b1'] = tf.Variable(tf.zeros([self.n_hidden[1]], dtype = tf.float32))

        all_weights['dec_w2'] = tf.get_variable("dec_w2", shape=[self.kernel_size[1], self.kernel_size[1], self.n_hidden[0],self.n_hidden[1]], #[3,3,10,20]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['dec_b2'] = tf.Variable(tf.zeros([self.n_hidden[0]], dtype = tf.float32))


        all_weights['dec_w3'] = tf.get_variable("dec_w3", shape=[self.kernel_size[0], self.kernel_size[0],1, self.n_hidden[0]], #[5,5,1,10]
            initializer=layers.xavier_initializer_conv2d(),regularizer = self.reg)
        all_weights['dec_b3'] = tf.Variable(tf.zeros([1], dtype = tf.float32))
    ...
    def encoder(self,x, weights):
        ...
        layer4 = tf.nn.bias_add(tf.nn.conv2d(layer3, weights['enc_w3'], strides=[1,2,2,1],padding='SAME'),weights['enc_b3'])
        layer4 = tf.nn.relu(layer4)

        return  layer4, shapes


    # Building the decoder
    def decoder(self,z, weights, shapes):
        # Encoder Hidden layer with relu activation #1
        #added 1st layer moved all the others
        ...
        layer4 = tf.add(tf.nn.conv2d_transpose(layer3, weights['dec_w3'], tf.stack([tf.shape(self.x)[0],shape_de4[1],shape_de4[2],shape_de4[3]]),\
         strides=[1,2,2,1],padding='SAME'),weights['dec_b3'])
        layer4 = tf.nn.relu(layer4)
...
if __name__ == '__main__':
    ...
    img = data['Y'] #2016x64x38
    I = []
    Label = []
    for i in range(img.shape[2]): #38 subjects of the YaleB dataset
        for j in range(img.shape[1]): #64 face images per subject: 2432 total images
            temp = np.reshape(img[:,j,i],[42,48]) #reshape each of the 2016 samples from 64x38 to 42x48
            Label.append(i) #append index
            I.append(temp)
    I = np.array(I)
    Label = np.array(Label[:])
    Img = np.transpose(I,[0,2,1]) #first dimension as is - second permuted across columns - third permuted across rows
    Img = np.expand_dims(Img[:],3) #add fourth dimension

    # face image clustering
    n_input = [48,42]
    kernel_size = [5,3,3,2]
    n_hidden = [10,20,30,30] #channels

    #pick K subjects (64 images each) to test robustness
    #Taking all possible combinations of K subjects out of 38 would result in too many experimental trials
    #To get a manageable size of experiments, we first number the subjects from 1 to 38 and then take all possible K consecutive subjects
    #e.g K=10: Images from subject 1-10, 2-11 ... 29-38
    #a total of 29 experimental trials

    all_subjects = [10, 15, 20, 25, 30, 35, 38] #combinations of experiments
    #finetuning 50+25K epochs
    avg = [] #Mean
    med = [] #Median

    iter_loop = 0
    while iter_loop < len(all_subjects):
        num_class = all_subjects[iter_loop] #number of subjects
        batch_size = num_class * 64 #64 face images per subject - should change this to a variable
        reg1 = 1.0 #L1 regularizer
        reg2 = 1.0 * 10 ** (num_class / 10.0 - 3.0) #L2 regularizer
        #model_path = './pretrain-model-EYaleB/model-102030-48x42-yaleb.ckpt'
        model_path = './pretrain-model-EYaleB/model-0-48x42-yaleb.ckpt'
        #restore_path = './pretrain-model-EYaleB/model-102030-48x42-yaleb.ckpt'
        restore_path = './pretrain-model-EYaleB/model-0-48x42-yaleb.ckpt'
        logs_path = './logs'
